...
from pyspark.sql.functions import (
    col, trim, regexp_replace, coalesce, to_date, abs as spark_abs,
    when, isnull, count, lit, percentile_approx, month, dayofweek,
    current_timestamp, row_number, desc, first, mode as spark_mode,
    broadcast, expr
)
...
def validate_schema(df, expected_cols, table_name):
    ...
    if missing:
        raise Exception(f"SCHEMA MISMATCH in {table_name}: missing columns {missing}")
