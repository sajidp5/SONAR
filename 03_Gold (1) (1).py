...
def check_rating_threshold(table_name, threshold=2.0):
    ...
    df = spark.table(table_name)
    low = df.filter(col("avg_rating") < threshold)
    if low.count() > 0:
...
def check_id_format(table_name, id_col, pattern):
    ...
    df = spark.table(table_name)
    bad = df.filter(df[id_col].rlike(pattern) != True).count()
    if bad > 0:
...
def check_null_percentage(table_name, max_pct=30.0):
    ...
    for c in numeric_cols:
        ...
        if null_pct > max_pct:
            failed.append(f"  • {c}: {null_pct:.1f}% nulls (threshold: {max_pct}%)")
    if failed:
        raise ValueError(
...
for adls_folder, tbl in DELTA_TABLES:
    ...
    try:
        ...
        df.write.format("delta").mode("overwrite").save(dbfs_path)
...
for tbl in GOLD_PARQUET:
    ...
    try:
        df = spark.table(tbl)
        row_count = df.count()
        df.write.format("parquet").mode("overwrite").save(dbfs_path)
        uploaded = 0
...
failed = [r for r in export_log if "❌" in r[3]]
if failed:
    print(f"\n⚠️  {len(failed)} table(s) had errors. Others completed successfully.")
else:
